...
1. GitHub Actions 在工作日北京时间 10:30、11:30、14:30 拉取长江有色价格，并在 16:30 收盘后同步广期所碳酸锂结算价。
...
碳酸锂数据来自广期所行情数据页面背后的公开日行情接口。任务会过滤碳酸锂小计行，对当天全部真实 LC 合约的结算价计算简单均值，然后更新碳酸锂历史缓存、日度预测、月度预测和网页数据库。该接口不需要 Cookie 或 GitHub Secret。
